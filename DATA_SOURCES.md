...
- 禁用 Tushare，包括 replay、本地代理、自定义 HTTP 地址、Token、默认链路和回退链路。禁止自动改用东方财富、BaoStock、网页抓价或其他来源；AKShare 只允许用于本规则指定的新浪分钟 K 线。
...
- 使用现有同花顺授权与开源 AKShare，不增加付费数据源；开源许可证不代表账号额度无限或永久免费。
